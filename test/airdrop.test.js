...
describe("AirDrop", function () {
  ...
  before(async function () {
    ...
    await Promise.all(this.shuffle.map(async (i, index) => {
      const receipt = await (await this.ethSwap.connect(addrs[i]).buyTokens({
        value: toWei(10)
      })).wait() // Each account buys 10,000 tokens worth 10 eth
      expect(receipt.blockNumber).to.eq(index + 2)
    }))

    // Query all tokensPurchases events between contract block number cut off on the ethSwap contract
    ...
    // Get elligible addresses from events and then hash them to get leaf nodes
  ...
  it("Only eligible accounts should be able to claim airdrop", async function () {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (proof.length !== 0) {
        ...
        await expect(this.airDrop.connect(addrs[i]).claim(prood)).to.be.revertedWith("Incorrect merkle proof")
